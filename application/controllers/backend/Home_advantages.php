...
class Home_advantages extends BackEndController
{
    protected $add;
    ...
    public function __construct()
    {
        ...
        $this->add = 'Добавить Диагностика';
        $this->index_title = 'Диагностика';
        $this->success_add = 'Вы успешно добавили новый Диагностика!';
        $this->success_update_order = 'Вы успешно обновили порядок отображения!';
        $this->success_edit = 'Вы успешно обновили Диагностика!';
        $this->success_delete = 'Вы успешно удалили Диагностика!';

        $this->data['title'] = $this->index_title;
        $this->data['add'] = $this->add;
        $this->load->model('home_advantages_model');
    }
    ...
    public function index()
    {
        ...
        $objects = $this->home_advantages_model->find();
        $this->data['inner_view'] = 'backend/home/advantages/index';
    ...
    public function put()
    {
        ...
        try {
            ...
            $id = $this->home_advantages_model->put($post);
            if (!$id) {
    ...
    public function update_order()
    {
        ...
        try {
            ...
            if (!$this->home_advantages_model->update_sorder($post)) {
                throw new Exception('Ошибка записи данных в таблицу: ' . $this->main_page);
            }
    ...
    public function item($id = 0)
    {
        ...
        $item = $this->home_advantages_model->findFirst($id);
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            try {
	            ...
	            if (!$this->home_advantages_model->update($post, $id)) {
                    throw new Exception('Ошибка записи данных в таблицу: ' . $this->main_page);
                }
        ...
        $this->data['inner_view'] = 'backend/home/advantages/item';
    ...
    public function delete($id = false)
    {
        ...
        try {
	        $keys = ['imgRU', 'imgRO','imgEN'];
	        foreach ($keys as $key) {
		        $fileList = recDirSearch($_SERVER['DOCUMENT_ROOT'] . '/public/home_advantages/', $item->{$key});
	        ...
	        if (!$this->home_advantages_model->delete($id)) {
