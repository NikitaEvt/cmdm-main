...
class Home_banner extends BackEndController
{
    protected $add;
    ...
    public function __construct()
    {
        ...
        $this->add = 'Добавить Баннер';
        $this->index_title = 'Баннер';
        $this->success_add = 'Вы успешно добавили новый Баннер!';
        $this->success_update_order = 'Вы успешно обновили порядок отображения!';
        $this->success_edit = 'Вы успешно обновили Баннер!';
        $this->success_delete = 'Вы успешно удалили Баннер!';
    ...
    public function index()
    {
        ...
        $objects = $this->home_banner_model->find();
        $this->data['inner_view'] = 'backend/home/banner/index';
    ...
    public function put()
    {
        ...
        try {
            ...
            $id = $this->home_banner_model->put($post);
            if (!$id) {
    ...
    public function update_order()
    {
        ...
        try {
            ...
            if (!$this->home_banner_model->update_sorder($post)) {
                throw new Exception('Ошибка записи данных в таблицу: ' . $this->main_page);
            }
    ...
    public function item($id = 0)
    {
        ...
        $item = $this->home_banner_model->findFirst($id);
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            try {
                ...
                if (!empty($_FILES['imgEN']['name'])) {
                    $this->upload->do_upload('imgEN');
                    $file_data = $this->upload->data();
                    $file = $file_data['file_name'];
	            ...
	            if (!$this->home_banner_model->update($post, $id)) {
                    throw new Exception('Ошибка записи данных в таблицу: ' . $this->main_page);
                }
        ...
        $this->data['inner_view'] = 'backend/home/banner/item';
    ...
    public function delete($id = false)
    {
        ...
        try {
	        ...
	        foreach ($keys as $key) {
		        $fileList = recDirSearch($_SERVER['DOCUMENT_ROOT'] . '/public/home_banner/', $item->{$key});
	        ...
	        if (!$this->home_banner_model->delete($id)) {
