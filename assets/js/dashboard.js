$(document).ready(function () {
    ...
    $('.mine_change_check').change(function () {
        ...
        id = $(this).val();
        col = $(this).data('col');
        $.post('/backend/change_check/', {table: table, id: id, value: value, col : col}, function (data) {
            try {
    ...
    $('.mine_change_check_firebase').change(function () {
        ...
        id = $(this).val();
        col = $(this).data('col');
        $.post('/backend/change_check_firebase/', {table: table, id: id, value: value, col : col}, function (data) {
            try {
                response = JSON.parse(data);
                if (response.status == 200) {
                    toastr.success('Ok!!!');
    ...
    $('.mine_change_select').change(function () {
        ...
        col = $(this).data('col');
        $.post('/backend/change_select/', {table: table, id: id, value: value, col : col}, function (data) {
            try {
                response = JSON.parse(data);
                if (response.status == 200) {
                    toastr.success('Ok!!!');
    ...
    $(document).on('click', '#delete_related_product', function(event) {
        ...
        if (confirm('Вы уверены что хотите удалить?')) {
            ...
            $.post('/backend/products/delete_related_product/', {product_id: product_id, related_id: related_id}, function (data) {
                response = JSON.parse(data);
    ...
    $('#add_related_product').on('click', function(event) {
        ...
        $.post('/backend/products/add_related_product/', {related_id: related_id, product_id: product_id}, function (data) {
            response = JSON.parse(data);
    ...
    $('#match').on('keyup', function(){
        ...
        if(match != '') {
            ...
            $.post('/backend/products/autocomplete/', {match: match}, function (data) {
                response = JSON.parse(data);
    ...
    $('.mine_delete_photo').on('click', function (event) {
        ...
        if (confirm('Вы точно уверены, что хотите удалить данную фотографию?')) {
            ...
            var tb = self.attr('data-tb');
            var column = self.attr('data-column');
            var path = self.attr('data-path');
            $.post('/backend/delete_photo/', {table: table, id: id, tb: tb, column:column, path:path}, function (data) {
                response = JSON.parse(data);
                try {
                    if (response.status == 200) {
    ...
    $('.mine_delete_img_row').on('click', function (event) {
        ...
        if (confirm('Вы точно уверены, что хотите удалить данную фотографию?')) {
            var id = self.attr('data-id');
            var table = self.attr('data-table');
            $.post('/backend/delete_img_row/', {table: table, id: id}, function (data) {
                try {
                    response = JSON.parse(data);
                    if (response.status == 200) {
                        self.closest('.mt-element-card').remove();
    ...
    $('.mine_clear_file').on('click', function (event) {
        ...
        if (confirm('Вы точно уверены, что хотите удалить данный файл?')) {
            ...
            $.post('/backend/delete_file/', {table: table, lang: lang, id: id}, function (data) {
                try {
                    response = JSON.parse(data);
                    if (response.status == 200) {
                        self.closest('.mt-element-card').remove();
